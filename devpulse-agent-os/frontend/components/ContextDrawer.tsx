...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  const cls = p.includes('high') || p.includes('blocker') || p.includes('critical')
  ...
  return (
    <div className="border border-gray-100 rounded-lg p-3 bg-gray-50 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        {event.branch && !isPR && (
          <span className="text-[10px] text-gray-400">on <span className="font-mono text-gray-600">{event.branch}</span></span>
        )}
      ...
      {isPR && event.pr_title && (
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-gray-800">#{event.pr_number} — {event.pr_title}</span>
      ...
      {event.commits && event.commits.length > 0 && (
        <div className="flex flex-col gap-1 pl-1 border-l-2 border-slate-200">
...
export function ContextDrawer({ context, loading, onClose }: ContextDrawerProps) {
  ...
  const isVisible = loading || !!context;
              ...
              <div className="flex flex-col gap-3">
                ...
                {context.github.total_events === 0 ? (
                  <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center text-gray-400 text-sm">
                    ...
                    <div className="text-xs mt-1 text-gray-300">Push a commit mentioning "{context.issue_key}" to link it.</div>
                  </div>
                ) : (
