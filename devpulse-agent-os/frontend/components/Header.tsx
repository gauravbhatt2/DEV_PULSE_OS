...
export function Header({ lastSync, onRefresh, disabled, jiraConnected, githubConnected, slackConnected }: HeaderProps) {
  ...
  return (
    <header className="flex justify-between items-center pb-5 border-b border-gray-200">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {/* Slack status — purple badge, only visible when token resolves */}
        <div
          id="slack-status-badge"
          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border text-xs font-medium transition-all duration-300 ${
            slackConnected
        ...
        >
          ...
          <svg
            className={`w-3 h-3 flex-shrink-0 ${
              slackConnected ? 'text-purple-500' : 'text-gray-300'
            }`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
          ...
          <span className={slackConnected ? 'text-purple-600 font-semibold' : ''}>
            {slackConnected ? '• Slack Connected' : 'Slack Not Configured'}
          </span>
        </div>
